<template>
  ...
  >
    <h3 
      :id="titleId" 
      class="cropedLine step__title italic textCentered paddingContainer">
      {{ title }}
    </h3>
    <div class="whiteBackground stepContent paddingContainer stepContent__paragraph zeroTopMargin normalWeight">
      ...
      <h4 
        v-if="showBenifit" 
        class="italic textCentered stepContent__subtitle">Бонусы по достижению</h4>
      <ul v-if="showBenifit">
        <li 
...
export default {
  ...
  methods: {
    ...
    switchState(opened = false) {
      this.$set(this, "opened", opened);

      if (this.isNotMobile()) {
        this.$store.commit("business/swtichOpenedState", opened);
      }
    },
    open() {
      const isMobile = this.$store.state.isUserFromMobileOrientation;
